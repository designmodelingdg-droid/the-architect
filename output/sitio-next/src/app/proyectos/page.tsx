...
const PROYECTOS = [
  ...
  },
  {
    ...
    disciplinas: "EST · Evaluación sísmica",
    ...
    <>
      ...
      <Section>
            ...
            <Reveal key={p.code} delay={i * 0.05}>
              <article className="panel grid overflow-hidden lg:grid-cols-[1.1fr_0.9fr]">
                ...
                <div className="p-7 md:p-9">
                  <span className="font-mono-tech text-[10.5px] uppercase tracking-[0.15em] text-tinta-suave">{p.disciplinas}</span>
                  <h2 className="mt-2 text-2xl font-bold text-white md:text-3xl">{p.tipo}</h2>
                  <p className="mt-3 text-[14.5px] leading-relaxed text-tinta-suave">{p.desc}</p>
                  <ul className="mt-5 space-y-2">
                    {p.alcance.map((a) => (
                      <li key={a} className="flex gap-2.5 text-[13.5px] text-tinta/90">
                        <span aria-hidden className="mt-2 size-1 shrink-0 rounded-full bg-naranja-claro" />
                        {a}
                      </li>
                    ))}
                  </ul>
        ...
        <Reveal className="mx-auto mt-10 max-w-xl">
          <p className="panel px-6 py-4 text-center font-mono-tech text-[11px] uppercase tracking-[0.12em] text-tinta-suave">
            Dossier completo de proyectos disponible a solicitud →{" "}
            <a href="/contactos" className="text-naranja-claro hover:text-white">pídelo aquí</a>
          </p>
        </Reveal>
      </Section>
